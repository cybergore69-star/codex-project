...
export async function downloadImage(refs = []) {
  ...
  if (url.protocol !== 'https:' || url.hostname !== 'files.oaiusercontent.com' || url.port || url.username || url.password) {
    const host = url.hostname || '(missing hostname)';
    fail(`Unsupported file host: ${host}. Report only this hostname; do not share the full download link.`);
  }
